...
def test_when_given_a_filled_scenario():
    ...
    result = scenario.run(base_url='base_url', retry=3, delay=2.0)
    assert result.label == 'label'
    assert result.status == Status.UNSTABLE
    assert result.case_results == [sentinel.result1, sentinel.result2]

    case1.assert_called_once_with(base_url='base_url', retry=3, delay=2.0)
    case2.assert_called_once_with(base_url='base_url', retry=3, delay=2.0)
